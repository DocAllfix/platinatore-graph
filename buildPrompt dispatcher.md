---
source_file: "il-platinatore-ai/backend/src/services/prompt.builder.ts"
type: "code"
community: "RAG Architecture Concepts"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/RAG_Architecture_Concepts
---

# buildPrompt dispatcher

## Connections
- [[GuideType taxonomy (5 fissi migration 004)]] - `implements` [EXTRACTED]
- [[RAG hybrid pipeline (HNSW + RRF)]] - `rationale_for` [INFERRED]
- [[buildChallenge template]] - `calls` [EXTRACTED]
- [[buildCollectible template]] - `calls` [EXTRACTED]
- [[buildPlatinum template]] - `calls` [EXTRACTED]
- [[buildPromptContext]] - `shares_data_with` [EXTRACTED]
- [[buildTrophy template]] - `calls` [EXTRACTED]
- [[buildWalkthrough template]] - `calls` [EXTRACTED]
- [[sanitizeUserQuery (anti-injection)]] - `calls` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/RAG_Architecture_Concepts